...
build: *.go
	...
	go build -v -o build/bin/$(GOOS)/$(BINARY_NAME) $(GOBUILD_VERSION_ARGS)

run: build
	./build/bin/$(GOOS)/$(BINARY_NAME) -i testdev
...
docker-kind: docker
	kind load docker-image $(IMAGE_NAME):$(IMAGE_TAG) --name nginx
...
# assumes local insecure registry is running locally on port 5000
docker-kind-registry: docker
	docker tag $(IMAGE_NAME):$(IMAGE_TAG) localhost:5000/healthcat:$(IMAGE_TAG)
	docker tag localhost:5000/healthcat:$(IMAGE_TAG) localhost:5000/healthcat:dev
	docker push localhost:5000/healthcat:$(IMAGE_TAG)
	docker push localhost:5000/healthcat:dev


release: check test docker
	docker push $(IMAGE_NAME):$(IMAGE_TAG)
	docker tag $(IMAGE_NAME):$(IMAGE_TAG) $(IMAGE_NAME):$(REPO_VERSION)
